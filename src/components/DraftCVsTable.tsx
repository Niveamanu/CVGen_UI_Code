...
export default function DraftCVsTable() {
  ...
  // Handle edit - fetch CV data and navigate with it
  const handleEdit = async (cv: CVCollection) => {
    try {
      ...
      navigate('/cv-builder', { 
        state: { 
          cvData: response,
          isEditing: true,
          originalEmail: cv.email,
          originalVersion: cv.version
        } 
      });
    } catch (error) {
      console.error("Error fetching CV for editing:", error);
      // You might want to show a toast error here
    }
  };
